...
class OpenAIWrapper:
        ...
        if os.getenv("BRAINTRUST_API_KEY"):
            try:
                self.braintrust_logger = init_logger(
                    project="LLM Simulation Service", api_key=os.getenv("BRAINTRUST_API_KEY")
                )
            ...
            try:
                ...
                if "unsupported_country_region_territory" in error_message or "403" in error_message:
                    self.logger.log_error(
                        f"OpenAI geographic restriction detected (attempt {attempt + 1}/{self.max_retries}): {str(e)}",
                        exception=e,
                        extra_data=error_context,
                    )

                    raise Exception(
                        f"OpenAI API blocked due to geographic restrictions after {self.max_retries} attempts"
                    )

                # Log different types of OpenAI errors differently
                elif "rate_limit" in error_message:
                    self.logger.log_error(
                        f"OpenAI rate limit exceeded (attempt {attempt + 1}/{self.max_retries})",
                        exception=e,
                        extra_data=error_context,
                    )
                    ...
                    self.logger.log_error(
                        f"OpenAI quota exceeded (attempt {attempt + 1}/{self.max_retries})",
                        exception=e,
                        extra_data=error_context,
                    )
                elif "connection" in error_message or "network" in error_message:
                    self.logger.log_error(
                        f"OpenAI connection error (attempt {attempt + 1}/{self.max_retries})",
                        exception=e,
                        extra_data=error_context,
                    )
                else:
    ...
    async def chat_completion(
        ...
        return await self._make_request_with_retry(
            messages=messages, session_id=session_id, temperature=temperature, seed=seed, tools=tools
        )

    async def json_completion(
        self, messages: List[Dict[str, str]], session_id: str, temperature: float = 0.3, seed: Optional[int] = None
