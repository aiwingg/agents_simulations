...
class BatchProcessor:
    ...
    async def run_batch(self, batch_id: str, progress_callback: Optional[Callable] = None) -> Dict[str, Any]:
        ...
        self.logger.log_info(
            f"Using prompt specification {job.prompt_spec_name} for batch", extra_data={"batch_id": batch_id}
        )
    ...
    async def _process_single_scenario(
        ...
        async with self.semaphore:  # Limit concurrency
            try:
                ...
                self.logger.log_info(
                    f"Processing scenario {scenario_index}: {scenario_name}", extra_data={"batch_id": batch_id}
                )
                ...
                if conversation_result.get("status") == "completed":
                    ...
                    combined_result = {
                        ...
                        "score": 2
                        if conversation_result.get("partial_completion")
                        else 1,  # Better score if partially completed
                ...
                self.logger.log_info(
                    f"Completed scenario {scenario_index}: {scenario_name}",
                    extra_data={
                        "batch_id": batch_id,
                ...
                self.logger.log_error(
                    f"Failed to process scenario {scenario_index}", exception=e, extra_data=error_context
                )
                raise e

    async def _update_sub_progress(
        self, batch_id: str, scenario_index: int, stage: str, progress_lock: asyncio.Lock = None
    ):
    ...
    async def _update_detailed_progress(
        self, batch_id: str, scenario_index: int, stage: str, additional_progress: float
    ):
        ...
        if batch_id in self.active_jobs:
            ...
            self.logger.log_info(
                f"Sub-progress updated for batch {batch_id}",
                extra_data={
                    "scenario_index": scenario_index,
                    "stage": stage,
    ...
    async def _update_progress(self, batch_id: str, completed_count: int = None):
        ...
        if batch_id in self.active_jobs:
            ...
            self.logger.log_info(
                f"Progress updated for batch {batch_id}",
                extra_data={
                    "old_progress": old_progress,
                    "new_progress": job.progress_percentage,
