...
class ResultStorage:
    ...
    def save_batch_results_ndjson(self, batch_id: str, results: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            self.logger.log_info(
                f"Saved NDJSON results",
                extra_data={"batch_id": batch_id, "filepath": filepath, "result_count": len(results)},
            )
    ...
    def save_batch_results_csv(
        self, batch_id: str, results: List[Dict[str, Any]], prompt_version: str = "default"
    ) -> str:
    ...
    def save_batch_results_json(self, batch_id: str, results: List[Dict[str, Any]]) -> str:
        ...
        try:
            with open(filepath, "w", encoding="utf-8") as f:
                json.dump(
                    {"batch_id": batch_id, "export_timestamp": datetime.now().isoformat(), "results": results},
                    f,
                    ensure_ascii=False,
                    indent=2,
                )
    ...
    def generate_summary_report(self, batch_id: str, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "total_turns" in df.columns:
                ...
                turn_stats = {"mean": 0, "median": 0, "min": 0, "max": 0}
            ...
            if "duration_seconds" in df.columns:
                ...
                duration_stats = {"mean": 0, "median": 0, "min": 0, "max": 0, "total": 0}
            ...
            scenario_agg.columns = [
                "_".join(col).strip() if isinstance(col, tuple) else str(col) for col in scenario_agg.columns
            ]
            scenario_stats = scenario_agg.to_dict()
    ...
    def save_summary_report(self, summary: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.log_info(f"Saved summary report", extra_data={"batch_id": batch_id, "filepath": filepath})
