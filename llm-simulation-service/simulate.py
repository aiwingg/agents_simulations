...
async def run_batch_local(
    scenarios: List[Dict[str, Any]], output_dir: str, prompt_spec_name: str = "default_prompts"
) -> Dict[str, Any]:
...
def fetch_batch_results_via_api(
    batch_id: str, api_url: str = "http://localhost:5000", fmt: str = "json"
) -> Optional[str]:
    """Fetch batch results from the service API."""
    import requests

    try:
        response = requests.get(f"{api_url}/api/batches/{batch_id}/results", params={"format": fmt})
        if response.status_code == 200:
...
async def run_single_scenario(
    scenario: Dict[str, Any], output_dir: str, stream: bool = True, prompt_spec_name: str = "default_prompts"
) -> Dict[str, Any]:
...
async def run_batch_scenarios(
    ...
    batch_id = batch_processor.create_batch_job(
        scenarios=scenarios, prompt_version="cli-v1.0", prompt_spec_name=prompt_spec_name, use_tools=Config.USE_TOOLS
    )
...
Examples:
  ...
  python simulate.py prompts duplicate default_prompts my_copy
    ...
    duplicate_prompts_parser = prompts_subparsers.add_parser(
        "duplicate", help="Duplicate existing prompt specification"
    )
    ...
    if args.command == "run":
        ...
        if args.single is not None:
            ...
            result = asyncio.run(
                run_single_scenario(scenario, args.output_dir, stream=stream, prompt_spec_name=args.prompt_spec)
            )
            ...
            # Run batch
            result = asyncio.run(run_batch_scenarios(scenarios, args.output_dir, prompt_spec_name=args.prompt_spec))
