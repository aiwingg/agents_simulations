...
class TestConversationAdapter:
    ...
    def test_extract_conversation_history_with_tools(self):
        ...
        # Tool call and result should be merged into a single entry
        assert len(history) == 2

        tool_entry = history[1]
        assert tool_entry["speaker"] == "agent_sales_agent"
        ...
        assert "tool_results" in tool_entry
        assert len(tool_entry["tool_results"]) == 1
    ...
    def test_merge_tool_events_with_final_message(self):
        ...
        messages = [
            ...
            ToolCallSummaryMessage(source="agent", content="Неверный номер"),
            TextMessage(source="agent", content="Спасибо"),
        ]
        ...
        assert final_entry["speaker"] == "agent_agent"
        assert "tool_calls" in final_entry
        assert "tool_results" in final_entry
        assert final_entry["content"] == "Спасибо"
    ...
    def test_conversation_adapter_integration(self):
        ...
        messages = [
            ...
            TextMessage(
                source="sales_agent",
                content="I'll help you find the perfect meat products",
            ),
            ToolCallRequestEvent(source="sales_agent", content=[tool_call]),
            ToolCallExecutionEvent(
                source="tool",
            ...
            TextMessage(
                source="sales_agent",
                content="I found fresh ground beef for $12.99. Would you like to add it to your cart?",
            ),
        ...
        # Tool events should be merged, resulting in 5 entries (excluding system message)
        ...
        # Tool call and result merged into agent response
        ...
        assert "tool_results" in history[2]
        assert "BEEF001" in str(history[2]["tool_results"][0])

        # Final client tool call (end_call)
        assert history[4]["speaker"] == "client"
        assert "tool_calls" in history[4]
        assert history[4]["tool_calls"][0]["function"]["name"] == "end_call"
