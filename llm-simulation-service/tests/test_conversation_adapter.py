...
class TestConversationAdapter:
    ...
    def test_skip_tool_summary_message(self):
        """ToolCallSummaryMessage should be ignored to avoid duplicated results"""
        tool_call = create_function_call("call_1", "test_tool", "{}")
        execution_result = FunctionExecutionResult(
            call_id="call_1",
            content='"ok"',
            is_error=False,
        ...
        messages = [
            TextMessage(source="client", content="start"),
            ToolCallRequestEvent(source="agent", content=[tool_call]),
            ToolCallExecutionEvent(source="tool", content=[execution_result]),
            ToolCallSummaryMessage(source="agent", content='"ok"'),
            TextMessage(source="agent", content="done"),
        ]

        history = ConversationAdapter.extract_conversation_history(messages)

        # Summary message should not appear in history
        assert len(history) == 4
        results_entries = [h for h in history if h.get("tool_results")]
        assert len(results_entries) == 1
        assert results_entries[0]["tool_results"][0] == "ok"
